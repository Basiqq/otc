...
local holdMWMacro = macro(100, "True MagicWall", function()
  if table.getn(marked_tiles) ~= 0 then
    for i, tile in pairs(marked_tiles) do
      ...
      if tile:getPosition().z == posz() then
        if tile and tile:getText() == "MARKED" and tile:getTimer() <= 300 or (tile:getTopThing():getId() ~= 2129 and tile:getTopThing():getId() ~= 2129) then
