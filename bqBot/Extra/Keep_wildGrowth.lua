...
local holdMWMacro = macro(100, "True WildGrowth", function()
  if table.getn(marked_tiles) ~= 0 then
    for i, tile in pairs(marked_tiles) do
      ...
      if tile:getPosition().z == posz() then
        if tile and tile:getText() == "MARKED" and tile:getTimer() <= 300 or (tile:getTopThing():getId() ~= 37236 and tile:getTopThing():getId() ~= 37236) then
          useWith(3159, tile:getTopUseThing())
		  delay(100)
        end
      else
        table.remove(marked_tiles, tablefind(marked_tiles, tile))
